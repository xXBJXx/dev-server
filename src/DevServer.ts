...
export class DevServer {
            ...
            .command(
                ...
                'Set up dev-server in the current directory. This should always be called in the directory where the io-package.json file of your adapter is located.',
                {
                    ...
                    dryRun: {
                        ...
                        default: false,
                        description:
                            'Show the setup actions without changing files, installing packages or connecting remotely',
                    },
                },
                async args =>
                    await this.setup(
            ...
            .command(
                ...
                'Run ioBroker dev-server and start the adapter in "watch" mode. The adapter will automatically restart when its source code changes. You may attach a debugger to the running adapter.',
                {
                    ...
                    watchPath: {
                        ...
                        array: true,
                        description:
                            'Watch an additional local directory for adapter restarts (repeatable; paths are relative to the adapter root)',
                    },
                    noBrowserSync: {
                        type: 'boolean',
                        alias: 'b',
    ...
    private async setDirectories(argv: {
        ...
        if (existsSync(path.join(this.tempPath, 'package.json'))) {
            ...
            if (argv.dryRun) {
                this.log.notice(
                    `Dry-run: legacy profile data would be migrated from ${this.tempPath} to ${defaultProfileDir}`,
                );
            } else {
    ...
    async setup(
        ...
        if (dryRun) {
            const pkg = await readJson<Record<string, any>>(path.join(this.rootPath, 'package.json'));
    ...
    async watch(
        ...
        useBrowserSync = true,
        watchPath: string | string[] | undefined = undefined,
    ): Promise<void> {
