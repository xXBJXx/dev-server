...
export interface PortConflict extends PortDefinition {
    pids: number[];
}

export function parseWindowsListeningPorts(output: string): Map<number, Set<number>> {
    ...
    for (const line of output.split(/\r?\n/)) {
        ...
        const pids = result.get(port) ?? new Set<number>();
        pids.add(pid);
...
export async function getWindowsPortOwners(): Promise<Map<number, Set<number>>> {
...
export async function findPortConflicts(ports: readonly PortDefinition[]): Promise<PortConflict[]> {
    ...
    const [owners, listening] = await Promise.all([
        ...
        Promise.all(
            uniquePorts.map(async definition => ({ definition, listening: await isPortListening(definition.port) })),
        ),
    ]);
...
export function formatPortConflict(conflict: PortConflict): string {
