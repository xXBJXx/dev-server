...
export class Watch extends RunCommandBase {
    ...
    private async prepareLocalProfileForWatch(): Promise<void> {
        ...
        try {
            ...
            try {
                ...
                if (instance?.common?.enabled) {
                    this.log.warn(
                        `Disabling controller-managed ${this.adapterName}.0 before startup to prevent a duplicate adapter process.`,
                    );
        ...
        throw new Error(
            `The dev-server profile "${this.owner.profileName}" is already running on objects DB port ` +
                `${this.getPort(OBJECTS_DB_PORT_OFFSET)}. Stop the other dev-server process before starting watch again.`,
        );
    }

    private async startOfflineProfileDb(): Promise<DBConnection> {
        ...
        if (existingLocks.length) {
            ...
            if (profileStarted) {
                throw new Error(
                    `The dev-server profile "${this.owner.profileName}" started while checking its database locks.`,
                );
            }
    ...
    private handleAdapterConfigChange(instanceObject: any): void {
        if (this.exiting) {
            return;
        }
        if (instanceObject?.common?.enabled) {
            this.log.warn(
                `The controller-managed ${this.adapterName}.0 instance was enabled while watch mode is running. ` +
    ...
    private async handleNodemonDetailMsg(message: string): Promise<void> {
        ...
        try {
            ...
            this.log.warn(`Couldn't determine nodemon child process: ${error as Error}`);
