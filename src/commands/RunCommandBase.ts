...
import { findPortConflicts, formatPortConflict, type PortDefinition } from './portDiagnostics.js';
...
export abstract class RunCommandBase extends CommandBase {
    ...
    protected getStartupPorts(): PortDefinition[] {
        return [
    ...
    private async assertStartupPortsAvailable(): Promise<void> {
        ...
        throw new Error(
            `Cannot start dev-server profile "${this.owner.profileName}" because required ports are already in use: ` +
    ...
    private async createCombinedConfigProxy(
        ...
        if (useBrowserSync) {
            if (uiCapabilities.configType === 'html' || uiCapabilities.tabType === 'html') {
                ...
                app.use(
                    createProxyMiddleware([adminPattern, '/__dev_server_reload/**'], {
                ...
                app.use(
                    createProxyMiddleware(['/__dev_server_reload/**'], {
                        target: `http://127.0.0.1:${browserSyncPort}`,
                        // ws: true, // can't have two web-socket connections proxying to different locations
                    }),
                );
    ...
    private async createJsonConfigProxy(app: Application, useBrowserSync = true): Promise<void> {
        const jsonConfigFile = this.getJsonConfigPath();
        const adminUrl = `http://127.0.0.1:${this.getPort(HIDDEN_ADMIN_PORT_OFFSET)}`;
    ...
    private async startLiveReload(port: number, hasReact: boolean): Promise<LiveReloadServer> {
     ...
     */
    private setupJsonFileWatch(liveReload: LiveReloadServer, filePath: string, fileName: string): void {
