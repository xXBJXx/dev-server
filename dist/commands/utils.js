...
export function findDescendantProcesses(processes, parentPid) {
...
export async function getChildProcesses(parentPid) {
    let processes;
    if (process.platform === 'win32') {
        const options = { windowsHide: true, maxBuffer: 10 * 1024 * 1024 };
        let stdout;
        try {
            const command = 'Get-Process | ForEach-Object { $parentId = 0; try { if ($_.Parent) { $parentId = $_.Parent.Id } } catch {}; [PSCustomObject]@{ ProcessId = $_.Id; ParentProcessId = $parentId; Name = $_.ProcessName } } | ConvertTo-Json -Compress';
            ({ stdout } = await execFileAsync('pwsh.exe', ['-NoLogo', '-NoProfile', '-NonInteractive', '-Command', command], options));
        }
        catch {
            ...
            ({ stdout } = await execFileAsync('powershell.exe', ['-NoLogo', '-NoProfile', '-NonInteractive', '-Command', command], options));
        }
